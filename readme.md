...
* `BufferedReadStream` overrides `Read(Span<byte>)`. Upstream does not, and both of `MultipartReaderStream`'s read paths call exactly that, so every read there falls to the base `Stream` shim - a pooled array and a full extra copy of the payload, per read. `MultipartReaderStream` overrides it too, so reading a section into a span does not pay the same shim.
